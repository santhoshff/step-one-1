...
const GooeyNav = ({
  ...
  const [activeIndex, setActiveIndex] = useState(propActiveIndex !== undefined ? propActiveIndex : initialActiveIndex);
  ...
  const updateEffectPosition = (element: HTMLElement) => {
    ...
    // Copy only the text content of the element
  ...
  // Keep internal activeIndex state in sync with external activeIndex prop
  useEffect(() => {
  ...
  // Effect to update positions and trigger particles when activeIndex changes
  ...
  useEffect(() => {
    ...
    if (activeLi) {
      ...
      // Do not run particles on initial mount to avoid screen noise
          ...
          {items.map((item, index) => {
            ...
            return (
              <li key={index} className={isItemActive ? 'active' : ''}>
                ...
                >
                  {item.colorClass && (
                    <span className={`w-1.5 h-1.5 rounded-full ${item.colorClass} ${isItemActive ? 'animate-pulse' : ''}`} />
                  )}
                  {item.label}
                </a>
              </li>
